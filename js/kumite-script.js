...
function round_set() {
  ...
  // AO
  var aoSelect = document.getElementById("ao_nat_input");
  var aoFlag = aoSelect.options[aoSelect.selectedIndex].getAttribute("data-flag");
  document.getElementById("ao_flag_output").src = aoFlag;

  // AKA
  var akaSelect = document.getElementById("aka_nat_input");
  var akaFlag = akaSelect.options[akaSelect.selectedIndex].getAttribute("data-flag");
  document.getElementById("aka_flag_output").src = akaFlag;
...
// Clear Set (reset pemain + pertandingan total)
function clear_set() {
  const nbsp = "\u00A0";

  // reset dropdown negara (kalau ada)
  const akaNatSel = document.getElementById("aka_nat_input");
  const aoNatSel  = document.getElementById("ao_nat_input");
  if (akaNatSel) akaNatSel.selectedIndex = 0;
  if (aoNatSel)  aoNatSel.selectedIndex  = 0;

  // kosongkan input
  ["aka_name_input","aka_team_input","ao_name_input","ao_team_input"].forEach(id=>{
    const el = document.getElementById(id); if (el) el.value = "";
  });

  // kosongkan output nama/tim (pakai nbsp biar layout stabil)
  ["aka_name_output","aka_team_output","ao_name_output","ao_team_output"].forEach(id=>{
    const el = document.getElementById(id); if (el) el.textContent = nbsp;
  });

  // reset skor & state senshu/win
  totalAka = 0; totalAo = 0;
  senshuAka = 0; senshuAo = 0;
  document.getElementById("score_aka").innerHTML = totalAka;
  document.getElementById("score_ao").innerHTML  = totalAo;
  aka_senshu_off(); ao_senshu_off();
  aka_win_off();    ao_win_off();

  // reset penalti (balikkan placeholder juga)
  aka_pen1_off(); aka_pen2_off(); aka_pen3_off(); aka_pen4_off(); aka_pen5_off();
  ao_pen1_off();  ao_pen2_off();  ao_pen3_off();  ao_pen4_off();  ao_pen5_off();

  // reset timer tanpa memanggil StopClock() (agar tidak trigger timeout_win())
  clearInterval(timerApp);
  inputMinute = 0; inputSecond = 0; inputMilisecond = 0;
  document.getElementById("outputMinute").innerHTML     = "00";
  document.getElementById("outputSecond").innerHTML     = "00";
  document.getElementById("outputMilisecond").innerHTML = "0";
  // kembalikan input waktu ke default (opsional)
  const minIn = document.getElementById("inputMinute");
  const secIn = document.getElementById("inputSecond");
  if (minIn) minIn.value = "2";
  if (secIn) secIn.value = "00";
...
function setDanger(isDanger) {
  ["outputMinute","outputSecond","outputMilisecond"].forEach(id=>{
    const el = document.getElementById(id);
    if (el) el.classList.toggle("time-danger", isDanger);
  });
}

function updateTimeColor() {
  // merah mulai tepat saat 00:15.0, lalu tetap merah hingga 00:00.0
  const isDanger =
    (inputMinute === 0 && inputSecond === 15 && inputMilisecond === 0) ||
    (inputMinute === 0 && inputSecond < 15);
